...
def test_snapshot_active_and_stale_branches(mocker, mock_boto3_client):
    ...
    # Real process_agent_state runs (both the active and stale branches execute,
    # which is what we need for coverage); only the DB write is intercepted so
    # we never touch a real database. We assert the handler completes
    # successfully rather than inspecting record internals, so the test stays
    # robust to how records are enriched/built.
    with patch("src.snapshot.post_to_db"):
        resp = lambda_handler(_build_event(), context=None)
...
def _enrichment_for(eids):
    """Minimal _agent_data enrichment entry for each eid."""
    return {
        eid: {
            "supplier": "S",
            "siteName": "X",
...
def test_process_agent_state_active_and_stale_branches(mocker):
    """Covers the stale branch (lag_status=True) AND the active branch
    (lag_status=False) of process_agent_state in one call."""
    set_env_vars()
    import src.snapshot as snap

    # one fresh agent (active) + one epoch agent (stale)
    fresh_ms = datetime.now(timezone.utc).timestamp() * 1000
    agent_state = {
        "ACTIVE_EID": {
            "routingProfile": "r",
            "agentStatus": "AVAILABLE",
            "initiationMethod": "test",
            "contactState": "CONNECTED",
            "lastUpdate": fresh_ms,   # recent -> active branch
        },
        "STALE_EID": {
            "routingProfile": "r",
            "agentStatus": "AVAILABLE",
            "lastUpdate": 0,          # epoch -> stale branch
        },
    }

    # the function reads the module global _agent_data for enrichment
    snap._agent_data = _enrichment_for(["ACTIVE_EID", "STALE_EID"])

    captured = {}
    mocker.patch.object(
        snap, "send_to_postgres",
        side_effect=lambda data, ts, cl: captured.update(data=data),
    )

    snap.process_agent_state(
        agent_state, ts="2022-05-02T05:43:22Z", cyber_logger=mocker.MagicMock()
    )

    lag_values = {d["lag_status"] for d in captured["data"]}
    assert True in lag_values     # stale branch covered
    assert False in lag_values    # active branch covered


def test_process_agent_state_eid_log_cap_overflow(mocker):
    """Covers the EID_LOG_CAP overflow branch: >50 stale agents triggers the
    '(+N more)' suffix path in the summary log (lines ~234-244)."""
    set_env_vars()
    import src.snapshot as snap

    # 60 stale agents (> EID_LOG_CAP of 50)
    agent_state = {
        f"STALE_{i}": {
            "routingProfile": "r",
            "agentStatus": "AVAILABLE",
    ...
    snap._agent_data = _enrichment_for(list(agent_state.keys()))
    ...
    # all 60 flagged stale
    assert len(captured["data"]) == 60
    assert all(d["lag_status"] is True for d in captured["data"])
